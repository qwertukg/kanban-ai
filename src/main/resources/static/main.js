...
async function selectBoard(id) {
    currentBoardId = id;
    const view = await fetchJson(`/api/boards/${id}`);
    renderBoard(view);
...
function renderBoard(view) {
    ...
    view.columns.sort((a, b) => a.order - b.order).forEach(col => {
        ...
        colDiv.innerHTML = `<div class="card h-100 column" data-column="${col.id}">` +
            ...
            `</div>`;
        const body = colDiv.querySelector('.card-body');
        colDiv.addEventListener('dragover', (e) => { e.preventDefault(); });
        colDiv.addEventListener('drop', (e) => {
            e.preventDefault();
            const taskId = e.dataTransfer?.getData('text/plain');
            if (taskId)
                moveTask(Number(taskId), col.id);
        });
...
function renderBoardsAdmin(boards) {
    ...
    document.getElementById('boardCreate').onclick = async () => {
        ...
        const targetBranch = document.getElementById('boardBranch').value || 'main';
...
function renderColumnsAdmin(columns, board, agents) {
    ...
    document.getElementById('createCol').onclick = async () => {
        ...
        const order = Number(document.getElementById('colOrder').value || 1);
        const agentIdValue = document.getElementById('colAgent').value;
...
function renderTasksAdmin(tasks, columns) {
    ...
    document.getElementById('taskCreate').onclick = async () => {
        ...
        const columnId = Number(document.getElementById('taskColumn').value);
        if (!currentBoardId)
            return;
...
function renderAgentsAdmin(agents) {
    ...
    document.getElementById('agentCreate').onclick = async () => {
        ...
        const roleInstructions = document.getElementById('agentRole').value;
        const acceptanceCriteria = document.getElementById('agentCriteria').value;
        const globalInstructions = document.getElementById('agentGlobal').value;
...
async function renderSettings() {
    ...
    document.getElementById('saveSettings').onclick = async () => {
        const apiKey = document.getElementById('setKey').value;
        const repository = document.getElementById('setRepo').value;
        const defaultBranch = document.getElementById('setBranch').value;
        const globalAgentInstructions = document.getElementById('setGlobal').value;
...
function setupMenu() {
    document.querySelectorAll('[data-view]').forEach(el => {
        el.addEventListener('click', (event) => {
            ...
            if (target === 'agentsView')
                refreshAgents();
            if (target === 'settingsView')
                renderSettings();
            if (target === 'boardsView')
                loadBoards();
            if (target === 'columnsView' && currentBoardId)
                selectBoard(currentBoardId);
            if (target === 'tasksView' && currentBoardId)
                selectBoard(currentBoardId);
        });
    });
}
document.addEventListener('openTaskForm', () => {
    ...
    if (!currentBoardId)
        return;
    if (title) {
        fetchJson(`/api/boards/${currentBoardId}`).then(view => {
