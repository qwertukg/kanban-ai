import type { Agent, Board, Column, GitEvent, Task } from './types';
...
const boardList = document.querySelector<HTMLDivElement>('#board-list')!;
const boardTitle = document.querySelector<HTMLHeadingElement>('#board-title')!;
const kanban = document.querySelector<HTMLDivElement>('#kanban')!;
const gitEventsView = document.querySelector<HTMLDivElement>('#git-events')!;
...
async function loadBoards() {
    state.boards = await api<Board[]>('/api/boards');
...
async function refreshBoardData() {
    ...
    const [columns, tasks, agents, gitEvents] = await Promise.all([
        api<Column[]>(`/api/boards/${state.selectedBoardId}/columns`),
        api<Task[]>(`/api/boards/${state.selectedBoardId}/tasks`),
        api<Agent[]>(`/api/agents`),
        api<GitEvent[]>(`/api/git-events`)
    ]);
...
function renderForms() {
    const boardSelects = [
        document.querySelector<HTMLSelectElement>('#column-board'),
        document.querySelector<HTMLSelectElement>('#task-board')
    ];
    ...
    const columnSelect = document.querySelector<HTMLSelectElement>('#task-column');
    if (columnSelect) {
    ...
    const agentSelect = document.querySelector<HTMLSelectElement>('#column-agent');
    if (agentSelect) {
    ...
    const agentList = document.querySelector<HTMLDivElement>('#agent-list');
    if (agentList) {
        agentList.innerHTML = '';
        state.agents.forEach(agent => {
...
function renderAgent(agentId?: number | null): string {
    if (!agentId) return '';
...
function bindForms() {
    document.querySelector<HTMLButtonElement>('#refresh')?.addEventListener('click', loadBoards);
    document.querySelector<HTMLButtonElement>('#create-board')?.addEventListener('click', async () => {
        const name = (document.querySelector<HTMLInputElement>('#board-name')?.value || '').trim();
        const targetBranch = (document.querySelector<HTMLInputElement>('#board-target')?.value || '').trim();
        if (!name) return;
        await api('/api/boards', { method: 'POST', body: JSON.stringify({ name, targetBranch }) });
        (document.querySelector<HTMLInputElement>('#board-name')!).value = '';
        await loadBoards();
    });

    document.querySelector<HTMLButtonElement>('#create-column')?.addEventListener('click', async () => {
        const boardId = Number((document.querySelector<HTMLSelectElement>('#column-board')?.value));
        const name = (document.querySelector<HTMLInputElement>('#column-name')?.value || '').trim();
        const order = Number(document.querySelector<HTMLInputElement>('#column-order')?.value || '0');
        const agent = (document.querySelector<HTMLSelectElement>('#column-agent')?.value || '');
        ...
        (document.querySelector<HTMLInputElement>('#column-name')!).value = '';
        await refreshBoardData();
    });

    document.querySelector<HTMLButtonElement>('#create-task')?.addEventListener('click', async () => {
        const boardId = Number(document.querySelector<HTMLSelectElement>('#task-board')?.value);
        const columnId = Number(document.querySelector<HTMLSelectElement>('#task-column')?.value);
        const title = (document.querySelector<HTMLInputElement>('#task-title')?.value || '').trim();
        const description = (document.querySelector<HTMLTextAreaElement>('#task-description')?.value || '').trim();
    ...
    document.querySelector<HTMLButtonElement>('#create-agent')?.addEventListener('click', async () => {
        const name = (document.querySelector<HTMLInputElement>('#agent-name')?.value || '').trim();
        const roleInstructions = (document.querySelector<HTMLTextAreaElement>('#agent-role')?.value || '').trim();
        const acceptanceCriteria = (document.querySelector<HTMLTextAreaElement>('#agent-acceptance')?.value || '').trim();
        if (!name) return;
        await api('/api/agents', { method: 'POST', body: JSON.stringify({ name, roleInstructions, acceptanceCriteria }) });
        (document.querySelector<HTMLInputElement>('#agent-name')!).value = '';
        (document.querySelector<HTMLTextAreaElement>('#agent-role')!).value = '';
        (document.querySelector<HTMLTextAreaElement>('#agent-acceptance')!).value = '';
        await refreshBoardData();
    });

    document.querySelector<HTMLButtonElement>('#save-settings')?.addEventListener('click', async () => {
        const openAiKey = (document.querySelector<HTMLInputElement>('#settings-key')?.value || '').trim();
        const repositoryUrl = (document.querySelector<HTMLInputElement>('#settings-repo')?.value || '').trim();
        const targetBranch = (document.querySelector<HTMLInputElement>('#settings-target')?.value || '').trim();
